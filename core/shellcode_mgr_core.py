...
class shell_mgr:
	...
	def match_shellcodes(self):
		...
		try:
			...
			if match:
				encoded = match.groups()[0]
				dec_shellcode = str(b64decode(encoded))
				if self.handle_phpbased_shellcode(dec_shellcode):
					return True 
					
			### Match Linux/x86 shell reverse TCP exploits
			if self.displayShellCode:
				print "starting Linux/x86 shell reverse TCP matching ..."
				stdout.flush()
			match = self.decodersDict['/x86'].search( self.shellcode2 )
			if match:
				...
				port = unpack('!H',raw_port)[0]
				self.log_obj.log("found Linux/x86 shell reverse TCP shellcode (port: %s, ip: %s)" % (port, ip), 9, "info", False, True)
				self.resultSet['result'] = True
    				self.resultSet['host'] = ip
    				self.resultSet['port'] = port
			...
			if self.displayShellCode:
				print "starting simple_backdoors_exec matching ..."
				stdout.flush()
			...
			if match:
				...
				self.log_obj.log("found simple_backdoors_exec connectback shellcode (port: %s, ip: %s)" % (port, ip), 9, "info", False, True)
				self.resultSet['result'] = True
    				self.resultSet['host'] = ip
    				self.resultSet['port'] = port
    				self.resultSet['found'] = "connbackshell"
